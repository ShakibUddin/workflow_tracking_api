...
## Teams (first real feature to use RBAC)

### Q36: What's the Teams schema, and how does team status work?
**A:** `teams` (`id`, `title`, `description` TEXT nullable, `team_status_id` FK -> `lookup.id`) and `user_teams` (`user_id` FK -> `users.id`, `team_id` FK -> `teams.id`, unique pair) - schema given directly rather than designed from scratch. `team_status_id` reuses the existing generic `lookup` table with a new `TEAM_STATUS` type (`ACTIVE`/`INACTIVE` values, seeded the same way `USER_STATUS` was - see Q17), instead of a new dedicated status table or a Postgres enum - same tradeoff already accepted for `users.status`: DB-level FK integrity to *some* lookup row, but not to `type = 'TEAM_STATUS'` specifically, which stays an application-level guarantee (`team.service.js` always resolves status through `LOOKUP_TYPES.TEAM_STATUS`). A new team defaults to `ACTIVE` on create (mirroring how signup defaults a user to `ACTIVE`); `status` becomes editable on `PATCH /teams/:id`.

### Q37: How is access to team data authorized - is it just `authorize(...)` like everything else?
**A:** No - this is the first place a purely role-based check isn't enough, because "can I see this specific team" depends on *which* team, not just *who* the requester is. Creating/updating/deleting a team, and adding/removing members, are ordinary coarse checks (`authorize(PERMISSIONS.TEAM_CREATE)` etc., `ADMIN`-only per Q35's seeded grants). But `GET /teams/:id` and `GET /users/:userId/teams` have no `authorize(...)` on the route at all - the middleware has no way to know "is requester a member of team 5" or "is requester the same person as :userId 5". That resource-level check lives in `team.service.js` instead (`getTeamDetail`, `getTeamsForUser`): an ADMIN (`team:view_all`) can see anything, everyone else can only see their own membership. This is the boundary the plan drew between "coarse RBAC" and "resource ownership" - see Q33's explicit rule against building general resource-attribute policies; this is a one-off identity check, not a policy engine.

### Q38: Who can be added to a team, and why does `user_teams` allow a user with any role in principle?
**A:** The DB schema places no role restriction on `user_teams` membership (by design - it's a generic many-to-many, and a future feature might want to add non-EMPLOYEE members). The restriction that only `EMPLOYEE`-role users can be added lives entirely in `team.service.js#addMembers` (rejects with 400 otherwise), matching "search EMPLOYEE's by name add them to the team" from the feature request. A user holding *both* `EMPLOYEE` and `ADMIN` (roles are many-to-many, nothing stops this) is still addable - the check is "has the EMPLOYEE role," not "does not have the ADMIN role." Adding an already-current member is a silent no-op rather than a 409 - the picker UI calling this repeatedly (e.g. re-submitting a form) shouldn't have to special-case "already added."

### Q39: A seeder ran fine in isolation but broke a full `db:seed:all` run once `permissions` had more entries in it - what happened, and what's the rule now?
**A:** `20260919020003-seed-permissions.js` and `20260919020004-seed-role-permissions.js` (from the RBAC work, Q33) were written as `Object.values(PERMISSIONS)` - i.e. "whatever is in the shared enum right now." That was fine when `PERMISSIONS` only had 3 entries, but the moment this Teams work added 5 more keys to that *same* shared object, those two old seeders silently started trying to insert/grant the new team permissions too - discovered as a hard-to-explain unique-constraint failure on `team:create` during a full, from-empty `db:seed:all`, several seeder files after the actual team-permission seeder had already run cleanly using its own fixed local list.
The fix: both seeders now capture their own literal, closed list at authorship time (`[PERMISSIONS.USER_MANAGE, PERMISSIONS.ROLE_MANAGE, PERMISSIONS.SESSION_MANAGE]`) instead of reading the live enum. The rule going forward: a seeder is a historical record of what was known/decided *at the time it was written* - it must never derive its data from a shared constant that other, later code is expected to keep extending. `20260919020006-seed-team-permissions.js` was written correctly from the start (its own local `TEAM_PERMISSIONS` array), which is why only the two *older* files needed the fix.

---

## HTTP security headers

### Q40: Why `helmet()` on every request?
**A:** `app.use(helmet())` is the first middleware registered in `app.js`, before CORS/body-parsing/routes. It sets a curated set of security-related HTTP response headers (e.g. `Content-Security-Policy`, `Strict-Transport-Security`, `X-Content-Type-Options: nosniff`, `X-Frame-Options`) with sane defaults, closing off a class of low-effort attacks (clickjacking, MIME-sniffing, protocol downgrade) that cost nothing to prevent and are easy to forget to set by hand one header at a time.
It's a small, well-maintained library with no app-specific configuration needed for this API (no inline scripts/styles to special-case in a CSP), so the default policy was kept as-is rather than tuned - registering it first ensures the headers apply to every response, including error responses from `errorHandler.middleware.js`.

---

## 100% test coverage

### Q41: How is 100% coverage enforced, and what does it actually measure?
**A:** `jest.config.js` sets `collectCoverageFrom: ['src/**/*.js', '!src/server.js', '!src/database/**']` and `coverageThreshold: { global: { statements/branches/functions/lines: 100 } }`, run via `npm run test:coverage`. `src/server.js` (just calls `app.listen`) and `src/database/**` (migrations/seeders - one-shot scripts run by `sequelize-cli`, never `require`d by the app) are excluded for the same reason `tests/helpers/*` isn't in the measured set at all: none of them are code a request ever runs through, so instrumenting them would only pad the percentage without proving anything. Everything actually reachable from an HTTP request - routes, middleware, controllers, services, repositories, models, DTOs, utils, and the `src/config/*` bootstrap files - is measured and sits at 100% branches/statements/functions/lines.

### Q42: Some of these branches are defensive checks against states the schema/call-sites already make impossible. Were they tested or deleted?
**A:** Both, depending on which: a few were found to be genuinely *unreachable* and handled accordingly, everything else was made reachable and given a real test.
- **Deleted as dead code**: `refreshTokenRepository.revoke()` (single-token revoke) - grepped and confirmed nothing in `src/` ever called it, only `revokeActiveByFamilyId` (family-wide revoke) is used. Per this project's own rule against writing tests just to pad coverage on code nobody calls, it was removed instead.
- **`istanbul ignore` with a one-line reason, left in place**: two branches that are structurally unreachable *without breaking a real guarantee elsewhere* rather than just "not currently hit by a test":
  - `token.service.js#rotate`'s `if (!user)` guard after resolving `family.userId` - unreachable because `users.id` cascades through `sessions`/`token_families`/`refresh_tokens` (see the create-sessions/create-token-families migrations), so a family/session/token can never resolve while its user is gone. Reaching it for real would require manufacturing a corrupt DB state by disabling FK enforcement - a contrivance that would test nothing about the app, so the guard is kept (schemas change) but excluded from the count.
  - `logger.js`'s `Object.keys(meta).length ? ... : ''` empty-metadata branch - confirmed empirically (`tests/config/logger.test.js` plus a manual probe) that winston's `defaultMeta: { service: ... }` is merged into every single log call, so `meta` always has at least one key; the empty-string side is dead as long as `defaultMeta` is configured.
- **Made reachable, not ignored, everywhere else**: most `x || []` / `x.field || fallback` guards in the services (e.g. `user.roles || []` in `auth.service.js`/`token.service.js`, `requestingUser.permissions || []` and `team.members || []` in `team.service.js`) exist because a *caller* could theoretically pass an object with that association not loaded, even though every real repository call in this codebase always eager-loads it (see each repository's own `defaultIncludes`/`include`). These are real defensive code, just never hit by an integration test that only ever calls through the repositories' own default paths - so they're tested directly against the service/repository functions with a hand-built or `jest.spyOn`-stubbed input that omits the association, bypassing the repository layer that would otherwise always supply it. See `tests/services/teamServiceEdgeCases.test.js`, `tests/services/teamServicePermissionFallback.test.js`, `tests/services/authService.test.js`, and the `rotate`/`me` fallback tests in `tests/auth/refreshToken.test.js` and `tests/auth/me.test.js`.

### Q43: Doesn't this contradict Q27's "no mocking the DB/repositories" rule?
**A:** No - Q27 is specifically about not mocking Sequelize/repositories to *assert concurrency/transaction correctness*, because a mock can't prove anything about real locking/race behavior. Every test that exercises that kind of correctness (rotation, reuse detection, session-limit races, cascade deletes) still runs against the real Postgres test database with zero mocking, unchanged.
What's added here is narrower and different in kind: (1) `jest.spyOn(repository, 'method').mockResolvedValueOnce(...)` for exactly one call, to prove a service-level defensive fallback works when a caller-shaped input omits an association the repository would never actually omit - not a claim about DB behavior, a claim about the service's own null-safety; and (2) `jest.mock('.../service')` in `tests/controllers/errorPropagation.test.js`, to prove a controller's `catch (err) { next(err); }` actually forwards an unexpected failure instead of hanging - plumbing that has no meaningful "real" version to test against, since there's no legitimate way to make a plain `listTeams()` call fail on demand against a healthy real database. Neither replaces or weakens any of the existing real-DB integration coverage; both are additive, single-purpose, and scoped to one file each.
